...
const ICON_CHEVRON_LEFT  = `<svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor"><path d="M9.78 12.78a.75.75 0 0 1-1.06 0L4.47 8.53a.75.75 0 0 1 0-1.06l4.25-4.25a.751.751 0 0 1 1.042.018.751.751 0 0 1 .018 1.042L6.06 8l3.72 3.72a.75.75 0 0 1 0 1.06Z"/></svg>`;
const ICON_CHEVRON_RIGHT = `<svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor"><path d="M6.22 3.22a.75.75 0 0 1 1.06 0l4.25 4.25a.75.75 0 0 1 0 1.06l-4.25 4.25a.751.751 0 0 1-1.042-.018.751.751 0 0 1-.018-1.042L9.94 8 6.22 4.28a.75.75 0 0 1 0-1.06Z"/></svg>`;
...
/** Sidebar HTML — shared between index and doc pages */
const sidebar = (
...
/** Doc page — mirrors /preview/[id]/docs/[slug] */
const buildDocPage = (
  docTitle: string,
  collectionTitle: string,
  html: string,
  ...
  <div class="nd-body">
    ...
    <main class="nd-main">
      ...
      <div class="nd-content">
        ...
        <div class="nd-docnav">
          ${prev
            ? `<a href="${assetPrefix}docs/${prev.slug}.html">
                ${ICON_CHEVRON_LEFT}
                <span><span class="nd-docnav-label">Precedente</span>${prev.title}</span>
               </a>`
            : "<div></div>"}
          ${next
            ? `<a href="${assetPrefix}docs/${next.slug}.html" class="nd-docnav-next">
                <span><span class="nd-docnav-label">Successivo</span>${next.title}</span>
                ${ICON_CHEVRON_RIGHT}
               </a>`
            : "<div></div>"}
        </div>` : ""}
...
export const buildStaticSiteZip = async (collectionId: string) => {
  ...
  zip.file(
    "index.html",
    buildIndexPage(rendered.collection.title, rendered.collection.description, allDocs, "")
  );
  ...
  zip.file("assets/katex.min.css", await fs.readFile(path.join(katexDist, "katex.min.css"), "utf8"));
  for (const font of await fs.readdir(path.join(katexDist, "fonts"))) {
    zip.file(`assets/fonts/${font}`, await fs.readFile(path.join(katexDist, "fonts", font)));
  }
