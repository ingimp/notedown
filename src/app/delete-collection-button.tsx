...
export function DeleteCollectionButton({ collectionId, collectionTitle }: Props) {
  ...
  return (
    <div
      className="flex items-center gap-1.5 bg-red-50 border border-red-300 rounded-gh px-2 py-1"
    ...
    >
      ...
      <button
        ...
        disabled={deleting}
        className="text-gh-xs font-semibold text-white bg-red-600 hover:bg-red-700 px-2 py-0.5 rounded transition-colors disabled:opacity-60"
      >
        {deleting ? "…" : "Sì"}
      </button>
      <button
        onClick={() => setConfirm(false)}
        className="text-gh-xs font-semibold text-gh-fg-muted hover:text-gh-fg px-1 py-0.5 rounded transition-colors"
      >
        No
      </button>
    </div>
